...
**✨ Now supports Tailwind CSS v4** with CSS-first configuration and automatic content detection!

> [!NOTE]
> **Migrating from v3?** Tailwind v4 introduces breaking changes. The main differences:
>
> - Configuration is now done in CSS files using `@import "tailwindcss"` and `@theme {}` blocks
> - No more `tailwind.config.js` files are created
> - Automatic content detection eliminates the need for manual content configuration
...
```swift
.package(url: "https://github.com/tuist/SwiftyTailwind.git", .upToNextMinor(from: "0.6.0"))
```
...
### Initializing Tailwind CSS v4 Configuration

You can create a Tailwind CSS configuration file by running the [`initialize`](<https://swiftytailwind.tuist.io/documentation/swiftytailwind/swiftytailwind/initialize(directory:options:)>) function on the `SwiftyTailwind` instance. In Tailwind v4, configuration is done directly in CSS files using `@import` and `@theme` directives:

```swift
try await tailwind.initialize(options: .full)
```

This creates a `tailwind.css` file with the following v4 structure:
...
Your `Styles/app.css` file should use the v4 format:
